 ...
 */		document.frmCmt.name.disabled=false;      
 //댓글 수정하고 다시 올때: <body onload="resetOk()">, 댓글 수정 취소할때 reset 버튼 onclick
	...
	function  updateCmt(cmtidx,writer,content) {  //실제 댓글 수정은 content 컬럼만 , 화면에 값표시 함수
		var obj = document.frmCmt;
		obj.idx.value=cmtidx;   //수정할 컬럼의 idx값을 input 태그 value 에 설정합니다.
		obj.name.value = writer;  //
...
<hr>
 ...
 <!-- 댓글 시작 -->
 <!-- 댓글 입력 -->
 <!-- 요청 파라미터 action은 get, 다른 input 요소는 post 메소드로 전송 -->
 ...
 <table style="width:60%;margin: auto;">
 	...
 	<tr>
 		...
 		<input type="text" name="writer" class="input1"></td>
 	...
 	<div id="comment">
 			<div>
			 <span class="name">${cmt.writer} (${cmt.ip})</span>				
 			 <span class="now">
 			 <fmt:parseDate pattern="yyyy-MM-dd'T'HH:mm" value="${cmt.wdate }" var="reDate"/>
 			 <fmt:formatDate value="${reDate }" pattern="yyyy-MM-dd a hh:mm"/>  <!-- 출력패턴 -->
 			 </span>
 		...
 		<!-- updateCmt() :댓글 리스트 중에   선택한 인덱스의 name,content 값을 input의 value 로 합니다.-->
	 			 <a href="javascript:updateCmt('${cmt.idx}','${cmt.writer}','${content}')" style="cursor: pointer;text-decoration: none;">
